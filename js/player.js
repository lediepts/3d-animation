  ...
  info: {
    init: function () {
      $('.play-list .play').each(function () {
        ...
        $(this).html('<div class="album-thumb pull-left">' + albumart + '</div><div class="songs-info pull-left">' + title + '<div class="songs-detail">' + artist + '</div></div></div>');
      });
    },
    load: function (id, album, artist, title, albumart, mp3) {
      ...
      $('.album-art').css({ "background": "url(" + albumart + ") no-repeat center center", "opacity": 0.75 });
      ...
      if (mp3) {
        $('.audio').html('<audio id="player" class="music" data-id="' + id + '" autoplay><source src="' + mp3 + '" type="audio/mpeg" /></audio>');
      }
    }
  },
  autoplay: function () {
    var id, album, artist, albumart, title, mp3, totalTrack, currentTrack, obj;
    totalTrack = $('.play-list>a').length;
    currentTrack = Math.floor(Math.random() * totalTrack) + 1;
    obj = $('.play-list a:nth-child(' + currentTrack + ')');
    obj.addClass('active');
    id = obj.data('id');
    album = obj.data('album');
    artist = obj.data('artist');
    albumart = obj.data('albumart');
    title = obj.data('title');
    mp3 = obj.data('url');
    $('.play-position').text(currentTrack + ' / ' + totalTrack);
    //$('.album-art').html('<img src="'+albumart+'" />');
    $('.album-art').css({ "background": "url(" + albumart + ") no-repeat center center", "opacity": 0.75 });
    $('.current-info .song-album').html('<i class="fa fa-music"></i> ' + album);
    $('.current-info .song-title').html('<i class="fa fa-headphones"></i> ' + title);
    $('.current-info .song-artist').html('<i class="fa fa-user"></i> ' + artist);
